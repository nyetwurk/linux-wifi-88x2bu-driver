...
#ifndef alloca
#define alloca(l) __builtin_alloca(l)
#endif
		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	char *input = alloca(wrqu->length + 1);
			      ...
			      union iwreq_data *wrqu, char *extra)
{
	...
	u8 *input = alloca(wrqu->data.length + 1);
	u32 bt_coexist;
		    ...
		    union iwreq_data *wrqu, char *extra)
{
	...
	char *input = alloca(wrqu->data.length + 1);
	int	qAutoLoad = 1;
